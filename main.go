...
import (
	...
	fynewin "github.com/wenchezhao/EyeCare/fyne"
...
func main() {
	...
	// 检查是否支持系统托盘
	fynewin.ShowTrayMenu(a, cfg, iconR)

	// loop
	go func() {
		for {
			time.Sleep(cfg.GetRemindInterval()) // wait for the next reminder
			fynewin.ShowFullScreenPopup(a, cfg)
		}
	}()
